...
class SELT_pca_f(SELT):
    def __init__(self, params=None):
        params_ = self.default_parameters()
        if (params is not None):
            params_.update(params)
        super(SELT_pca_f, self).__init__(params_)
        self.name = "SELT_pca_f" 
        self.feature_embds = None

    def default_parameters(self): 
        params = super(SELT_pca_f, self).default_parameters()
        p = {
            "PCA_params": dict(copy=True, whiten=False, svd_solver='auto', tol=0.0, iterated_power='auto', n_oversamples=10, power_iteration_normalizer='auto', random_state=params["random_state"]),
        }
        params.update(p)
        return params
    ...
    def learn_embeddings(self, dataset):
        ...
        with np.errstate(invalid="ignore"): # for NaN or 0 variance matrices
            ...
            self.model_embeddings.fit(X)
        self.feature_embds = self.model_embeddings.transform(X)
        
    def transform(self, entity): ## entity of size F x n 
        assert self.feature_embds is not None
        return self.feature_embds.T.dot(entity.values)
...
if __name__ == "__main__":
	...
	model = SELT_pca_f({"n_dimensions": N_DIMENSIONS, "random_state": SEED, "fm_params": {"loss": "CrossEntropyLoss", "batch_size": BATCH_SIZE, "n_epochs": N_EPOCHS, "opt_params": {"lr":0.01, "weight_decay":0.02}}})
	model.fit(train)
