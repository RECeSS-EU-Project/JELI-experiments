...
dataset_params = dict(npoints=int(np.sqrt(29929)), nfeatures=10, ndim=2, sparsity_features=0.8, dvar=0.01, dmean=2, sparsity=0.9)
...
if __name__ == "__main__":
	...
	import pickle
	import os
	if (os.path.exists("%s/results_parameter.pck" % folder)):
		with open("%s/results_parameter.pck" % folder, "rb") as f:
	...
	for idd, dataset_type in enumerate(dataset_types):
		...
		if (os.path.exists("%s/results_parameter_%s.pck" % (folder, dataset_type))):
			with open("%s/results_parameter_%s.pck" % (folder, dataset_type), "rb") as f:
		...
		for iss, data_seed in enumerate(data_seeds):
			...
			if (os.path.exists("%s/results_parameter_%s_%d.pck" % (folder, dataset_type, data_seed))):
				with open("%s/results_parameter_%s_%d.pck" % (folder, dataset_type, data_seed), "rb") as f:
			...
			for isp, ndim in enumerate(ndims):
				...
				if (os.path.exists("%s/results_parameter_%s_%d_%d.pck" % (folder, dataset_type, data_seed, ndim))):
					with open("%s/results_parameter_%s_%d_%d.pck" % (folder, dataset_type, data_seed, ndim), "rb") as f:
				...
				for its, iter_seed in enumerate(iter_seeds):
					...
					if (os.path.exists("%s/results_parameter_%s_%d_%d_%d.pck" % (folder, dataset_type, data_seed, ndim, iter_seed))):
						with open("%s/results_parameter_%s_%d_%d_%d.pck" % (folder, dataset_type, data_seed, ndim, iter_seed), "rb") as f:
					...
					for imm, model_name in enumerate(models):
						...
						with open("%s/results_parameter_%s_%d_%d_%d.pck" % (folder, dataset_type, data_seed, ndim, iter_seed), "wb") as f:
							pickle.dump(results_algo, f)
		
					results_parameter_iter.update({iter_seed: results_algo})
					with open("%s/results_parameter_%s_%d_%d.pck" % (folder, dataset_type, data_seed, ndim), "wb") as f:
						pickle.dump(results_parameter_iter, f)
				results_parameter.update({ndim: results_parameter_iter })
				with open("%s/results_parameter_%s_%d.pck" % (folder, dataset_type, data_seed), "wb") as f:
					pickle.dump(results_parameter, f)
			results_data.update({dataset_params["data_seed"]: results_parameter})
			with open("%s/results_parameter_%s.pck" % (folder, dataset_type), "wb") as f:
				pickle.dump(results_data, f)
		results.update({dataset_type: results_data})
		with open("%s/results_parameter.pck" % folder, "wb") as f:
