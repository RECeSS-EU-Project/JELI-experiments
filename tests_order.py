...
proc.wait()
dataset_params = dict(npoints=int(np.sqrt(32*32)), nfeatures=10, ndim=2, sparsity_features=0.8, dvar=0.01, dmean=2, sparsity=0.9)
...
if __name__ == "__main__":
	...
	Ndata=50 #100      ## iterate over several randomly generated instances
	...
	import pickle
	import os
	if (os.path.exists("%s/results_order.pck" % folder)):
	...
	for idd, dataset_type in enumerate(dataset_types):
		...
		if (os.path.exists("%s/results_order_%s.pck" % (folder, dataset_type))):
			with open("%s/results_order_%s.pck" % (folder, dataset_type), "rb") as f:
		...
		for iss, data_seed in enumerate(data_seeds):
			...
			if (os.path.exists("%s/results_order_%s_%d.pck" % (folder, dataset_type, data_seed))):
				with open("%s/results_order_%s_%d.pck" % (folder, dataset_type, data_seed), "rb") as f:
					results_parameter = pickle.load(f)	
			else:
				results_parameter = {}
			
			for isp, order in enumerate(orders):
				print("*** Order %d (%d/%d) -- Data seed %d (%d/%d) -- Dataset type %s (%d/%d)" % (order, isp+1, len(orders), data_seed, iss+1, len(data_seeds), dataset_type, idd+1, len(dataset_types)))
				if (order in results_parameter):
					continue
				iter_seeds = np.random.choice(range(int(1e8)), size=Niter, replace=False)
				
				if (os.path.exists("%s/results_order_%s_%d_%d.pck" % (folder, dataset_type, data_seed, order))):
					with open("%s/results_order_%s_%d_%d.pck" % (folder, dataset_type, data_seed, order), "rb") as f:
						results_parameter_iter = pickle.load(f)	
				else:
					results_parameter_iter = {}
				
				for its, iter_seed in enumerate(iter_seeds):
					print("**** Iter seed %d (%d/%d) -- Order %d (%d/%d) -- Data seed %d (%d/%d) -- Dataset type %s (%d/%d)" % (iter_seed, its+1, len(iter_seeds), order, isp+1, len(orders), data_seed, iss+1, len(data_seeds), dataset_type, idd+1, len(dataset_types)))
					if (iter_seed in results_parameter_iter):
						continue
				
					if (os.path.exists("%s/results_order_%s_%d_%d_%d.pck" % (folder, dataset_type, data_seed, order, iter_seed))):
						with open("%s/results_order_%s_%d_%d_%d.pck" % (folder, dataset_type, data_seed, order, iter_seed), "rb") as f:
					...
					for imm, model_name in enumerate(models):
						print("***** Algorithm %s (%d/%d) -- Iter seed %d (%d/%d) -- Order %d (%d/%d) -- Data seed %d (%d/%d) -- Dataset type %s (%d/%d)" % (model_name, imm+1, len(models), iter_seed, its+1, len(iter_seeds), order, isp+1, len(orders), data_seed, iss+1, len(data_seeds), dataset_type, idd+1, len(dataset_types)))
						if (model_name in results_algo):
							continue

						if ("JELI"==model_name):
							algorithm_params["JELI"]["order"] = order
							res_metrics = train_test(train, test, "JELI", algorithm_params["JELI"], type_=0)
						else:
							raise ValueError
							
						results_algo.update({model_name: res_metrics})
						with open("%s/results_order_%s_%d_%d_%d.pck" % (folder, dataset_type, data_seed, order, iter_seed), "wb") as f:
							pickle.dump(results_algo, f)
		
					results_parameter_iter.update({iter_seed: results_algo})
					with open("%s/results_order_%s_%d_%d.pck" % (folder, dataset_type, data_seed, order), "wb") as f:
						pickle.dump(results_parameter_iter, f)
				results_parameter.update({order: results_parameter_iter })
				with open("%s/results_order_%s_%d.pck" % (folder, dataset_type, data_seed), "wb") as f:
					pickle.dump(results_parameter, f)
			results_data.update({dataset_params["data_seed"]: results_parameter})
			with open("%s/results_order_%s.pck" % (folder, dataset_type), "wb") as f:
				pickle.dump(results_data, f)
		results.update({dataset_type: results_data})
		with open("%s/results_order.pck" % folder, "wb") as f:
